...
<body class="row w-100" >
	...
	<!-- End Preloader -->
		...
		<hr class="bg-white">
  				...
  				<li class="list-item  py-2 bg-dark"><a href="Frequentclient.php" class="current text-center px-0">Frequent Client</a></li>
			...
			<div class="row">
				<div class="col-12">
					...
					<table class="table shopping-summery">
						...
						<tbody>
							<?php 
							$qf = "SELECT `email`,`name` FROM `ordertabel` GROUP BY `email` ORDER BY `email` DESC limit 10";
							$runf = mysqli_query($db,$qf);
							while ($simplef = mysqli_fetch_assoc($runf)) {
								
							 ?>
							<tr>
								<td class="product-des" data-title="Client Name">
									<p class="product-des text-center"><?php echo ucfirst($simplef['name']) ?></p>
								</td>
								<td class="product-des" data-title="Client Email">
									<p class="product-des text-center"><?php echo ucfirst($simplef['email']) ?></p>
								</td>
